...
import { getHospitalClaims, verifyHospitalClaim } from "../services/api";
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-slate-500">Hidden: {hidden?.length || 0}</span>
      </div>
      {entries.length ? (
        <div className="mt-3 space-y-3">
          {entries.map(([field, meta]) => (
            <div key={field} className="rounded-2xl bg-white p-3 shadow-sm">
              ...
              <div className="mt-1 break-all font-mono text-xs text-sky-700">{meta.hash}</div>
            </div>
          ))}
        </div>
      ) : (
...
function RejectionReasons({ checks, status }) {
  ...
  const reasons = [];
  if (!checks?.zkVerified) reasons.push("ZK Proof invalid");
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Hospital portal</p>
              <h1 className="mt-2 text-3xl font-bold text-slate-900">Patient-approved hash review</h1>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Total Claims" value={stats.total} note="Hospital queue" />
          <StatCard label="Pending" value={stats.pending} note="Need verification" />
          <StatCard label="Approved" value={stats.approved} note="Auto-cleared" />
          <StatCard label="Rejected" value={stats.rejected} note="Failed checks" />
        </div>

        {error ? <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div> : null}
        {loading ? <div className="rounded-[28px] border border-slate-200 bg-white px-6 py-12 text-center text-slate-500 shadow-sm">Loading hospital claims...</div> : null}

        {!loading && !claims.length ? (
          <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-6 py-12 text-center text-slate-500 shadow-sm">No claims available.</div>
        ) : null}

        {!loading && claims.map((claim) => (
          <div key={claim.id} className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Claim ID</div>
  ...
  {"*".repeat(12)}
                 ...
                 <div className="mt-3 flex flex-wrap items-center gap-2">
                   ...
                   {claim.status === "Pending" && (
                     ...
                     >
                       {busyId === claim.id ? "Verifying..." : "Verify Claim"}
                     </button>
                   )}
                 </div>
              </div>
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-3">
              ...
              <div className="flex flex-col gap-4 xl:col-span-1">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 h-full">
                  <div className="font-semibold text-slate-900">Proof summary</div>
                  <div className="mt-2">
                    <div className="flex flex-col gap-1">
                      <span>Public signals: <span className="font-mono text-xs text-slate-700">{claim.publicSignals?.join(", ") || "-"}</span></span>
                      <span className="text-[10px] text-slate-400 italic">(1: Pass, 0: Fail | Order: Amount, Treatment, Time, Duplicate)</span>
                    </div>
                  </div>
                  <div className="mt-2">Statement: amount upper bound {claim.proofStatement?.amountUpperBound || "-"}, within bound {String(claim.proofStatement?.amountIsWithinBound ?? false)}</div>
                  <div className="mt-2">Hospital verified: {String(claim.hospitalVerified)}</div>
                  <div className="mt-2">ZK verified: {String(claim.zkVerified)}</div>
                </div>
              </div>
              <div className="flex flex-col gap-4 xl:col-span-1">
                <RejectionReasons checks={claim.checks} status={claim.status} />
              </div>
            </div>
          </div>
        ))}
