...
import { getPatient, getClaims, submitClaim, savePatient, deleteClaim, recoverClaim, updateClaimPermissions, updateClaim, permanentDeleteClaim, permanentDeleteAllClaims, recoverAllClaims } from "../services/api";
...
export default function PatientDashboard() {
  ...
  const handleRecoverAllClaims = async () => {
    if (window.confirm("Are you sure you want to recover all deleted claims?")) {
      try { await recoverAllClaims(PATIENT_ID); loadData(); } catch (e) { alert(e.message); }
    }
  };

  const handlePermanentDeleteAll = async () => {
    if (window.confirm("Are you sure you want to delete all deleted claims forever? This action cannot be undone.")) {
      try { await permanentDeleteAllClaims(PATIENT_ID); loadData(); } catch (e) { alert(e.message); }
    }
  };
