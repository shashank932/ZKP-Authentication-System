...
app.get(["/patient/:id", "/api/patient/:id"], async (req, res) => {
  try {
...
app.get(["/patient/claims/:id", "/api/patient/claims/:id"], async (req, res) => {
  try {
...
app.put("/api/claims/:id", async (req, res) => {
  try {
    ...
    // Calculate remaining coverage amount excluding the current claim (if it was already approved)
    const approvedClaims = await Claim.find({
      patientId: claim.patientId,
      status: "Approved",
      isDeleted: { $ne: true },
...
app.get(["/hospital/claims", "/api/hospital/claims"], async (req, res) => {
  try {
...
app.post(["/hospital/verify/:id", "/api/hospital/verify/:id"], async (req, res) => {
  try {
    const claim = await Claim.findOne({ id: req.params.id });
    if (!claim) return res.status(404).json({ error: "Claim not found" });
...
app.get(["/insurance/claims", "/api/insurance/claims"], async (req, res) => {
  try {
    const claims = await Claim.find({ patientId: "PAT-001", isDeleted: { $ne: true } });
    const result = await Promise.all(claims.map(async (claim) => {
      const patient = await Patient.findOne({ id: claim.patientId });
...
app.post(["/insurance/verify/:id", "/api/insurance/verify/:id"], async (req, res) => {
  try {
    const claim = await Claim.findOne({ id: req.params.id });
    if (!claim) return res.status(404).json({ error: "Claim not found" });
...
app.delete(["/claims/permanent-all/:patientId", "/api/claims/permanent-all/:patientId"], async (req, res) => {
  try {
...
app.post(["/claims/recover-all/:patientId", "/api/claims/recover-all/:patientId"], async (req, res) => {
  try {
